...
///////////////////////////////////////////////////////////////////
// Date: 2014/5/14                                               //
// Factorized Personalized Markov Chain with Pairwise Learning   //
    ...
    void train() {
        ...
        for (int i=0; i<niters; i++) {
            ...
            for (vector<TrainPair*>::iterator it=tr_pairs->begin();
                ...
                #pragma omp parallel for
                for (int j=0; j<ndim; j++) {
                    user_factor[uidx] = user_factor[uidx][j] + lr*guser_factor[j];
                    poi_factor[pidx] = poi_factor[pidx][j] + lr*gpoi_factor[j];
                    poi_factor[pidx1] = poi_factor[pidx1][j] + lr*gpoi_factor1[j];
                    poi_factor[pidx2] = poi_factor[pidx2][j] + lr*gpoi_factor2[j];
                }
