...
//                                                               //
// Note: restricte User_Context to be diagonal matrix, this is   //
//   equivalent to Query.*User_Context.*Item+User*Item           //
//                                                               //
///////////////////////////////////////////////////////////////////
...
public:
    ...
    void factorInit() {
        int num_para=0;
        if (bias_tag)
            num_para = (2*n_users+n_pois+n_querys)*ndim+n_pois;
        else
            num_para = (2*n_users+n_pois+n_querys)*ndim;
        ...
        for (int u=0; u<n_users; u++) {
            user_context_factor[u] = total_factor+ind;
            utils::muldimGaussrand(user_context_factor[u], ndim);
            //utils::muldimUniform(user_context_factor[u], ndim);
            //uitls::muldimZero(user_context_factor[u], ndim);
            ind += ndim;
        }
            ...
            for (vector<TrainPair*>::iterator it=tr_pairs->begin();
                ...
                if (bias_tag) {
                    p_val = utils::dot(user_factor[uidx], poi_factor[pidx1], ndim)
                          + utils::elewiseProduct(query_factor[qidx], user_context_factor[uidx],
                ...
                for (int j=0; j<ndim; j++) {
                    ...
                    gquery_factor[j] = logit_loss*user_context_factor[uidx][j]*t_diff[j]
                                       -q_reg*query_factor[qidx][j];
                    guser_context_factor[j] = logit_loss*query_factor[qidx][j]*t_diff[j]
                                              -uc_reg*user_context_factor[uidx][j];
                    t_sum[j] = user_factor[uidx][j]+query_factor[qidx][j]*user_context_factor[uidx][j];
                ...
                for (int j=0; j<ndim; j++) {
                    ...
                    user_context_factor[uidx][j] = user_context_factor[uidx][j] + lr*guser_context_factor[j]; 
        ...
        while (getline(*in, line)) {
            ...
            for (vector<string>::iterator it=candidate_pois->begin();
                ...
                score = utils::dot(user_factor[user_ids[uid]], poi_factor[poi_ids[c_pid]], ndim)
                      + utils::elewiseProduct(query_factor[query_ids[poi_query[pid1]]],
    ...
    void saveModel() {
        ...
        for (int u=0; u<n_users; u++)
            fwrite(user_context_factor[u], sizeof(double), ndim, f);
        fclose(f);
    ...
    void loadModel() {
        ...
        for (int u=0; u<n_users; u++)
            utils::fread_(user_context_factor[u], sizeof(double), ndim, f);
        fclose(f);
