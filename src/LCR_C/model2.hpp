...
//   Query*User_Context*Item+User*Item                           //
//                                                               //
// Note: User_Context is a complete matrix                       //
//                                                               //
///////////////////////////////////////////////////////////////////
...
public:
    ...
    ~LCR() {
        ...
        if (poi_factor)
            delete[] poi_factor;
        if (user_context_factor) {
            for (int u=0; u<n_users; u++)
                delete user_context_factor[u];
            delete[] user_context_factor;
        }
    ...
    void factorInit() {
        int num_para=0;
        if (bias_tag)
            num_para = (n_users+n_pois+n_querys+n_users*ndim)*ndim+n_pois;
        else
            num_para = (n_users+n_pois+n_querys+n_users*ndim)*ndim;
        ...
        user_context_factor = new double**[n_users];
        for (int u=0; u<n_users; u++) {
                user_context_factor[u] = new double *[ndim];
            for (int k=0; k<ndim; k++) {
                user_context_factor[u][k] = total_factor+ind;
                utils::muldimGaussrand(user_context_factor[u][k], ndim);
                //utils::muldimUniform(user_context_factor[u][k], ndim);
                //uitls::muldimZero(user_context_factor[u][k], ndim);
                ind += ndim;
            }
        }
    ...
    void train() {
        ...
        double ** guser_context_factor = new double*[ndim];
        for (int i=0; i<ndim; i++)
            guser_context_factor[i] = new double[ndim];
            ...
            for (vector<TrainPair*>::iterator it=tr_pairs->begin();
                ...
                if (bias_tag) {
                    p_val = utils::dot(user_factor[uidx], poi_factor[pidx1], ndim)
                          + utils::vecProMatProVec(query_factor[qidx], user_context_factor[uidx],
                                  poi_factor[pidx1], ndim)
                          + poi_bias[(*it)->pidx1];
                    n_val = utils::dot(user_factor[uidx], poi_factor[pidx2], ndim)
                          + utils::vecProMatProVec(query_factor[qidx], user_context_factor[uidx],
                                  poi_factor[pidx2], ndim)
                          + poi_bias[(*it)->pidx2];
                } else {
                    p_val = utils::dot(user_factor[uidx], poi_factor[pidx1], ndim)
                          + utils::vecProMatProVec(query_factor[qidx], user_context_factor[uidx],
                ...
                memset(gquery_factor, 0, sizeof(double)*ndim);
                memset(gpoi_factor1, 0, sizeof(double)*ndim);
                memset(gpoi_factor2, 0, sizeof(double)*ndim);
                // compute factor gradient
                #pragma omp parallel for
                for (int j=0; j<ndim; j++) {
                    for (int k=0; k<ndim; k++) {
                        gquery_factor[j] += user_context_factor[uidx][j][k]*poi_factor[pidx1][k]
                                          - user_context_factor[uidx][j][k]*poi_factor[pidx2][k];
                        guser_context_factor[j][k] = query_factor[qidx][j]*poi_factor[pidx1][k]
                                                   - query_factor[qidx][j]*poi_factor[pidx2][k];
                        gpoi_factor1[j] += query_factor[qidx][k]*user_context_factor[uidx][k][j];;
                    }
                    t_diff[j] = poi_factor[pidx1][j]-poi_factor[pidx2][j];
                    guser_factor[j] = logit_loss*t_diff[j]
                                      -u_reg*user_factor[uidx][j];
                    gquery_factor[j] = logit_loss*gquery_factor[j]
                                       -q_reg*query_factor[qidx][j];
                    for (int k=0; k<ndim; k++)
                        guser_context_factor[j][k] = logit_loss*guser_context_factor[j][k]
                                              -uc_reg*user_context_factor[uidx][j][k];
                    gpoi_factor2[j] = -logit_loss*(user_factor[uidx][j]+gpoi_factor1[j])
                                      -p_reg*poi_factor[pidx2][j];
                    gpoi_factor1[j] = logit_loss*(user_factor[uidx][j]+gpoi_factor1[j])
                ...
                for (int j=0; j<ndim; j++) {
                    ...
                    for (int k=0; k<ndim; k++)
                        user_context_factor[uidx][j][k] = user_context_factor[uidx][j][k]
                                                        + lr*guser_context_factor[j][k]; 
    ...
    void recommendation(char* submission_path) {
        ...
        while (getline(*in, line)) {
            ...
            for (vector<string>::iterator it=candidate_pois->begin();
                ...
                c_pid = *it;
                score = utils::dot(user_factor[user_ids[uid]], poi_factor[poi_ids[c_pid]], ndim)
                      + utils::vecProMatProVec(query_factor[query_ids[poi_query[pid1]]],
                            user_context_factor[user_ids[uid]], poi_factor[poi_ids[c_pid]], ndim);
                if (bias_tag)
    ...
    void saveModel() {
        ...
        for (int u=0; u<n_users; u++)
            for (int i=0; i<ndim; i++)
                fwrite(user_context_factor[u][i], sizeof(double), ndim, f);
        fclose(f);
    ...
    void loadModel() {
        ...
        for (int u=0; u<n_users; u++)
            for (int k=0; k<ndim; k++)
                utils::fread_(user_context_factor[u][k], sizeof(double), ndim, f);
        fclose(f);
