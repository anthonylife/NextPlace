...
###################################################################
# Date: 2014/5/8                                                  #
# Location aware Latent Collaborative Retrieval with Pairwise     #
...
from tool import logitLoss, getPoiCategoryInfo
...
class LLCR():
    ...
    def model_init(self, trdata_path, vadata_path, tedata_path, category_path,
        ...
        init_para = None
        if init_choice == settings["LLCR_INIT_ZERO"]:
            init_para = rZero
        elif init_choice == settings["LLCR_INIT_GAUSSIAN"]:
            init_para = rGaussian
        else:
    ...
    def load_model(self):
        ...
        self.ruser_ids = {}
        u_cnt = 0
        for entry in csv.reader(open(settings["LLCR_USER_FACTOR_FILE"])):
            uid = int(entry[0])
        ...
        self.rpoi_ids = {}
        p_cnt = 0
        for entry in csv.reader(open(settings["LLCR_POI_FACTOR_FILE"])):
            pid = int(entry[0])
        ...
        self.rcategory_ids = {}
        c_cnt = 0
        for entry in csv.reader(open(settings["LLCR_CATEGORY_FACTOR_FILE"])):
            cid = int(entry[0])
        ...
        if self.bias_tag == True:
            self.poi_bias = np.array([0.0 for i in xrange(self.poi_ids)])
            for entry in csv.reader(open(settings["LLCR_POI_BIAS_FILE"])):
...
def main():
    ...
    llcr.model_init(trdata_path,
                    ...
                    grid_path,
                    para.data_num,
                    settings["LLCR_INIT_GAUSSIAN"],
                    bias_tag,
                    restart_tag)
    llcr.train()
    #llcr.recommendation(submission_path)
    llcr.recommendationNewPOI(submission_path)
