...
///////////////////////////////////////////////////////////////////
// Date: 2014/5/20                                               //
// Location aware Latent Collaborative Retrival with Pairwise    //
//   Learning for implicit feedback data.                        //
// Factorization Format:                                         // 
//   (Query*User_Context1+Location*User_Context2)*Item+User*Item //
///////////////////////////////////////////////////////////////////
...
class LLCR{
    ...
    double * total_factor;
    double ** user_factor;
    double ** user_context_factor1;
    double ** user_context_factor2;
    double ** poi_factor;
    double ** query_factor;
    double * poi_bias;
...
public:
    ...
    LLCR(char* trdata_path, char* vadata_path, char* tedata_path,
    ...
    void factorInit() {
        int num_para=0;
        if (bias_tag)
            num_para = (n_users+n_pois+n_querys+2*n_users)*ndim+n_pois;
        else
            num_para = (n_users+n_pois+n_querys+2*n_users)*ndim;
        ...
        user_context_factor1 = new double*[n_users];
        for (int u=0; u<n_users; u++) {
            user_context_factor1[u] = total_factor+ind;
            utils::muldimGaussrand(user_context_factor1[u], ndim);
            //utils::muldimUniform(user_context_factor1[u], ndim);
            //uitls::muldimZero(user_context_factor1[u], ndim);
            ind += ndim;
        }
        
        user_context_factor2 = new double*[n_users];
        for (int u=0; u<n_users; u++) {
            user_context_factor2[u] = total_factor+ind;
            utils::muldimGaussrand(user_context_factor2[u], ndim);
            //utils::muldimUniform(user_context_factor2[u], ndim);
            //uitls::muldimZero(user_context_factor2[u], ndim);
            ind += ndim;
        }
    ...
    void train() {
        ...
        double * guser_context1 = new double[ndim];
        double * guser_context2 = new double[ndim];
        ...
        for (int i=0; i<niters; i++) {
            ...
            for (vector<TrainPair*>::iterator it=tr_pairs->begin();
                ...
                pidx2 = (*it)->pidx2;
                if (bias_tag) {
                    p_val = beta1*utils::dot(user_factor[uidx], poi_factor[pidx1], ndim)
                          + beta2*utils::dot(query_factor[qidx], user_context_factor1[uidx], poi_factor[pidx1], ndim)
                          + beta2*utils::dot(poi_factor[pidx], user_context_factor2[uidx], poi_factor[pidx1], ndim)
                          + poi_bias[(*it)->pidx1];
                    n_val = beta1*utils::dot(user_factor[uidx], poi_factor[pidx2], ndim)
                          + beta2*utils::dot(query_factor[qidx], user_context_factor1[uidx], poi_factor[pidx2], ndim)
                          + beta2*utils::dot(poi_factor[pidx], user_context_factor2[uidx], poi_factor[pidx2], ndim)
                          + poi_bias[(*it)->pidx2];
                } else {
                    p_val = beta1*utils::dot(user_factor[uidx], poi_factor[pidx1], ndim)
                          + beta2*utils::dot(query_factor[qidx], user_context_factor1[uidx], poi_factor[pidx1], ndim)
                ...
                for (int j=0; j<ndim; j++) {
                    ...
                    guser_factor[j] = beta1*logit_loss*t_diff[j]
                                    - u_reg*user_factor[uidx][j];
                    gquery_factor[j] = beta2*logit_loss*user_context_factor1[uidx][j]
                                     * t_diff[j]-q_reg*query_factor[qidx][j];
                    gpoi_factor[j] = beta2*logit_loss*user_context_factor2[uidx][j]
                                   * t_diff[j]-p_reg*poi_factor[pidx][j];
                    guser_context1[j] = beta2*logit_loss*query_factor[qidx][j]
                                      * t_diff[j]-uc_reg*user_context_factor1[uidx][j];
                    guser_context2[j] = beta2*logit_loss*poi_factor[pidx][j]
                                      * t_diff[j]-uc_reg*user_context_factor2[uidx][j];

                    t_sum[j] = beta1*user_factor[uidx][j]
                             + beta2*user_context_factor1[uidx][j]*query_factor[qidx][j]
                             + beta2*user_context_factor2[uidx][j]*poi_factor[pidx][j];
                ...
                for (int j=0; j<ndim; j++) {
                    ...
                    user_context_factor1[uidx][j] = user_context_factor1[uidx][j]
                                                  + lr*guser_context1[j];
                    user_context_factor2[uidx][j] = user_context_factor2[uidx][j]
                                                  + lr*guser_context2[j];
                }
        ...
        delete[] gpoi_factor1;
        delete[] gpoi_factor2;
        delete[] guser_context1;
        delete[] guser_context2;
        delete tr_pairs; 
    }
    ...
    void recommendation(char* submission_path) {
        ...
        while (getline(*in, line)) {
            ...
            for (vector<string>::iterator it=candidate_pois->begin();
                ...
                c_pid = *it;
                score = beta1*utils::dot(user_factor[user_ids[uid]], 
                            poi_factor[poi_ids[c_pid]], ndim)
                      + beta2*utils::dot(query_factor[query_ids[poi_query[pid1]]],
                            user_context_factor1[user_ids[uid]], poi_factor[poi_ids[c_pid]], ndim)
                      + beta2*utils::dot(poi_factor[poi_ids[pid1]],
                            user_context_factor2[user_ids[uid]], poi_factor[poi_ids[c_pid]], ndim);
                if (bias_tag)
    ...
    void saveModel() {
        ...
        for (int u=0; u<n_users; u++)
            fwrite(user_context_factor1[u], sizeof(double), ndim, f);
        for (int u=0; u<n_users; u++)
            fwrite(user_context_factor2[u], sizeof(double), ndim, f);
        fclose(f);
    ...
    void loadModel() {
        ...
        for (int u=0; u<n_users; u++)
            utils::fread_(user_context_factor1[u], sizeof(double), ndim, f);
        for (int u=0; u<n_users; u++)
            utils::fread_(user_context_factor2[u], sizeof(double), ndim, f);
        fclose(f);
