...
###################################################################
# Date: 2014/5/7                                                  #
# Social Collaborative Retrieval with Pairwise Learning for       #
...
class SCR():
    ...
    def model_init(self, trdata_path, vadata_path, tedata_path, category_path,
        ...
        init_para = None
        if init_choice == settings["SCR_INIT_ZERO"]:
            init_para = rZero
        elif init_choice == settings["SCR_INIT_GAUSSIAN"]:
            init_para = rGaussian
        else:
    ...
    def train(self):
        ...
        for i in xrange(self.niters):
            ...
            for j, triple in enumerate(self.tr_triples):
                if self.bias_tag == True:
                    pos_score = np.dot(self.user_factor[triple[0]]+self.query_factor[triple[1]], self.poi_factor[triple[2]])\
                              + self.poi_bias[triple[2]]
                    neg_score = np.dot(self.user_factor[triple[0]]+self.query_factor[triple[1]], self.poi_factor[triple[3]])\
                ...
                self.user_factor[triple[0]] = self.user_factor[triple[0]]\
                        +self.lr*(logit_loss*tmp_diff1-self.u_reg*self.user_factor[triple[0]]-self.alpha*(self.user_factor[triple[0]]-self.fsum_factor))
                self.query_factor[triple[1]] = self.query_factor[triple[1]]+self.lr*(logit_loss*tmp_diff1-self.q_reg*self.query_factor[triple[1]])
                tmp_diff2 = self.user_factor[triple[0]] + self.query_factor[triple[1]]
    ...
    def recommendation(self, submission_path):
        ...
        for i, entry in enumerate(csv.reader(open(self.tedata_path))):
            ...
            sys.stdout.write("\rFINISHED RECOMMENDATION TRAINING NUM: %d. " % (i+1))
            sys.stdout.flush()
    ...
    def load_model(self):
        ...
        self.ruser_ids = {}
        u_cnt = 0
        for entry in csv.reader(open(settings["SCR_USER_FACTOR_FILE"])):
            uid = int(entry[0])
        ...
        self.rpoi_ids = {}
        p_cnt = 0
        for entry in csv.reader(open(settings["SCR_POI_FACTOR_FILE"])):
            pid = int(entry[0])
        ...
        self.rcategory_ids = {}
        c_cnt = 0
        for entry in csv.reader(open(settings["SCR_CATEGORY_FACTOR_FILE"])):
            cid = int(entry[0])
        ...
        if self.bias_tag == True:
            self.poi_bias = np.array([0.0 for i in xrange(self.poi_ids)])
            for entry in csv.reader(open(settings["SCR_POI_BIAS_FILE"])):
...
def main():
    ...
    if para.data_num == 0:
        ...
        friendship_path = settings["ROOT_PATH"] + settings["SRC_DATA_FILE1_2"]
        ...
        friendship_path = settings["ROOT_PATH"] + settings["SRC_DATA_FILE2_2"]
        ...
        friendship_path = settings["ROOT_PATH"] + settings["SRC_DATA_FILE3_2"]
    ...
    submission_path = settings["ROOT_PATH"] + settings["SCR_SUBMISSION_PATH"]
    restart_tag = True
    bias_tag = False
    ...
    scr.model_init(trdata_path,
                    ...
                    grid_path,
                    para.data_num,
                    settings["SCR_INIT_GAUSSIAN"],
                    bias_tag,
                    restart_tag)
    scr.train()
    #scr.recommendation(submission_path)
    scr.recommendationNewPOI(submission_path)
