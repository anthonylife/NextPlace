...
///////////////////////////////////////////////////////////////////
// Date: 2014/5/14                                               //
// Social regularized Factorized Personalized Markov Chain with  //
// Pairwise Learning for implicit feedback data.                 //
///////////////////////////////////////////////////////////////////
...
public:
    void paraInit(){
        ...
        vadata_path = NULL;
        tedata_path = NULL;
        frienddata_path = NULL;
        user_factor_path = NULL;
        poi_factor_path = NULL;
        poi_bias_path = NULL;
    }
    
    SFPMC(char* trdata_path, char* vadata_path, char* tedata_path,
            char* frienddata_path, char* user_factor_path, char* poi_factor_path,
            char* poi_bias_path, char* poi_path, char* grid_path, int data_num,
            bool bias_tag, bool restart_tag, int re_topk){
        paraInit();
        this->trdata_path = trdata_path;
        this->vadata_path = vadata_path;
        this->tedata_path = tedata_path;
        this->frienddata_path = frienddata_path;
        ...
        friend_sim = utils::loadUserSimInfo(frienddata_path, &user_ids);
    ...
    void train() {
        ...
        double * fsum_factor = new double[ndim];
        ...
        for (int i=0; i<niters; i++) {
            ...
            for (vector<TrainPair*>::iterator it=tr_pairs->begin();
                ...
                // compute logistic loss (This can be replaced by EXP value table)
                logit_loss = utils::logitLoss(p_val-n_val);
                
                // compute sum factor of top similar users (friends)
                memset(fsum_factor, 0, sizeof(double)*ndim);
                for (unsigned int j=0; j<friend_sim[uidx].friends.size(); j++) {
                    t_uidx = friend_sim[uidx].friends[j];
                    //#pragma omp parallel for
                    for (int k=0; k<ndim; k++)
                        fsum_factor[k] += user_factor[t_uidx][k]
                                          *friend_sim[uidx].simvals[j];
                }

                // compute factor gradient (Add social regularization for user)
                #pragma omp parallel for
                for (int j=0; j<ndim; j++) {
                    t_diff[j] = poi_factor[pidx1][j]-poi_factor[pidx2][j];
                    guser_factor[j] = logit_loss*t_diff[j]
                                      -u_reg*user_factor[uidx][j]
                                      -alpha*(user_factor[uidx][j]-fsum_factor[j]);
                    gpoi_factor[j] = logit_loss*t_diff[j]
